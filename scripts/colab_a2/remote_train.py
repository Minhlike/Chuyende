...
validates durable Google Drive state, generates launch authorization, and executes/resumes runs.
"""
...
def build_authorization_payload(seed: int) -> str:
    return f"""
import json, hashlib
...
auth_data = {{
    ...
    "execution_code_commit_sha": "{FROZEN_EXECUTION_COMMIT}",
...
def authorize_and_launch_seed(bridge: ColabCLIBridge, seed: int, resume_checkpoint: Optional[str] = None, resume_sha256: Optional[str] = None) -> Tuple[bool, str]:
    """Generates authorization artifact and launches empirical training for a single seed."""
    if seed not in CANONICAL_SEEDS:
        raise ValueError(f"Seed {seed} not in canonical list: {CANONICAL_SEEDS}")
    
    # 1. Authorize
    print(f"[AUTH] Authorizing Seed {seed} on remote runtime...")
    auth_code = build_authorization_payload(seed).strip()
