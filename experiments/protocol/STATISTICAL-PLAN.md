...
| :--- | :--- | :--- | :--- |
| **Detection Performance** | PR-AUC, Macro-F1, Recall@0.1% FPR | **Absolute $\Delta$** ($\Delta = 	ext{Score}_{	ext{proposed}} - 	ext{Score}_{	ext{baseline}}$) | Relative $\Delta$ percentage ($rac{\Delta}{	ext{Score}_{	ext{base}}} 	imes 100\%$) |
| **Operational Latency** | p50, p95, p99 Latency (ms) | **Absolute $\Delta	ext{ ms}$** ($\Delta t = t_{	ext{proposed}} - t_{	ext{baseline}}$) | Latency Ratio ($rac{t_{	ext{proposed}}}{t_{	ext{baseline}}}$) |
| **Memory / State Size** | Peak RAM, State Size (MiB) | **Absolute $\Delta	ext{ MiB}$** ($\Delta M = M_{	ext{proposed}} - M_{	ext{baseline}}$) | Compression Ratio ($rac{M_{	ext{proposed}}}{M_{	ext{baseline}}}$) |
...
| :--- | :--- | :--- | :--- |
| **Family 1 (H1 — Fidelity)** | 4 dynamic parameter attack subtypes (SQLi, Command Injection, Path Traversal, Port Scan). | **Bonferroni Adjustment** | Adjusted significance level $lpha' = rac{0.05}{4} = 0.0125$. |
| **Family 2 (H2 — Multi-View)** | 3 pairwise component tests (Multi-View vs Seq-only, Multi-View vs Graph-only, Multi-View vs Unaligned). | **Bonferroni Adjustment** | Adjusted significance level $lpha' = rac{0.05}{3} = 0.0167$. |
| **Family 3 (H3 — Robustness)** | 12 perturbation operators (P01..P12). | **Benjamini-Hochberg FDR** | Target False Discovery Rate $q^* = 0.05$. |
| **Family 4 (H4 — Operational)** | 3 conjunctive SLO targets (Latency $\le 10	ext{ms} \land 	ext{RAM} \le 500	ext{MB} \land 	ext{Throughput} \ge 10^4$). | **Intersection-Union Test (IUT)** | Strict conjunctive compliance (all must pass at $lpha = 0.05$, no $p$-value hunting). |
