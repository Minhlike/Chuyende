...
Đánh giá xem các biểu diễn đa góc nhìn (multi-view) có được căn chỉnh z_mv góc nhìn (view) đơn chiếm ưu thế Pareto hay không
...
def evaluate_h2_multiview_alignment_contract(
    ...
    # So sánh 1: góc nhìn (view) nhiều chế độ được căn chỉnh và chỉ theo trình tự
    ...
    # So sánh 2: đa góc nhìn (multi-view) được căn chỉnh so với chỉ đồ thị
    ...
    # Căn chỉnh phải được hỗ trợ ít nhất là không được căn chỉnh và không tệ hơn các góc nhìn (view) đơn lẻ
    is_falsified = (
