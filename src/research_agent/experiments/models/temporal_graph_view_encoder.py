...
for Stage A2 Self-Supervised Pretraining on Event-Entity Logs (Contract V1.3).
...
Invariants:
  ...
  2. Relation Target Firewall: Prediction input [h_v(t-) || h_u(t-) || phi(delta_t)]
     strictly withholds true relation ID and relation embedding.
  3. Node Target Firewall: Reconstruction head predicts x_v_fixed_priv in R^6 strictly
     from h_v(t-) without direct target vector pass-through.
  4. Causal Degree Targets: Degree features computed at t- prior to edge increment.
  5. Inductive Split Reset: Full dynamic memory and interaction state zeroed on boundary.
"""
...
class TemporalGraphViewEncoder(nn.Module):
    """
    Temporal Graph View Encoder using GRUCell memory and Temporal Multi-Head Attention.
    """
    def __init__(
        self,
        d_node: int = 128,
        ...
        d_type_emb: int = 32,
        dropout: float = 0.10,
        num_relations: int = 9,       # 0: UNK, 1..8: Canonical HDFS relations
        ...
        self.relation_embedding = nn.Embedding(num_relations, d_rel_emb)
        ...
        # Message Generator: [h_src || h_dst || e_edge || e_rel || phi(dt)] -> d_msg
        msg_in_dim = d_node + d_node + d_edge + d_rel_emb + d_time_proj
        ...
        # 1. Relation Classification Head: [h_v(t-) || h_u(t-) || phi(dt)] -> num_relations
        ...
        self.loss_node_fn = nn.SmoothL1Loss(beta=1.0)
    ...
    def forward_event_window(
        ...
        Processes a temporal event window sequentially, strictly enforcing:
          ...
          4. Post-prediction GRU memory updates
        """
        if not events:
        ...
        for event in events:
            ...
            dst = event["dest_node"]
            src_type = event["source_type"]
            dst_type = event["dest_type"]
            rel_id = event["relation_id"]
            ...
            # Target representations for node reconstruction
            ...
            # 1a. Masked Edge Relation Prediction Head (Target withheld from input)
            rel_in = torch.cat([h_src_2d, h_dst_2d, phi_dt], dim=-1) # (1, rel_in_dim)
            rel_logits = self.rel_head(rel_in)                       # (1, num_relations)
            if mask_rel:
                target_rel_tensor = torch.tensor([rel_id], dtype=torch.long, device=device)
                loss_rel_list.append(self.loss_rel_fn(rel_logits, target_rel_tensor))
                masked_rel_count += 1

            # 1b. Masked Node Feature Reconstruction Head (Target withheld from input)
            if mask_node_src:
            ...
            # Message generation for src and dst
            msg_src_in = torch.cat([h_src_2d, h_dst_2d, e_edge, e_rel, phi_dt], dim=-1)
            msg_dst_in = torch.cat([h_dst_2d, h_src_2d, e_edge, e_rel, phi_dt], dim=-1)
            ...
            # Save updated states (detached dynamic memory for causal state progression)
            self.node_memory[src] = h_src_new.detach()
            self.node_memory[dst] = h_dst_new.detach()
