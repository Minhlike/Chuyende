<!--
DOCUMENT_IDENTITY:
  document_type: "Chuyên đề chuyên sâu"
  author_role: "Sinh viên"
  degree_level: "NOT_INFERRED"
  masters_thesis: false
  doctoral_dissertation: false
  graduation_thesis: false
-->

# BÁO CÁO TỔNG KẾT TÍCH HỢP BẰNG CHỨNG THỰC NGHIỆM NINEPLUS VÀO CHUYÊN ĐỀ CHUYÊN SÂU
**Mã báo cáo:** SPECIALIZED-TOPIC-NINEPLUS-INTEGRATION-REPORT  
**Ngày hoàn thành:** 2026-09-18  
**Trạng thái nghiệm thu:** HOÀN THÀNH TÍCH HỢP  
...
| :--- | :--- | :--- | :--- |
| **Trạng thái tập Kiểm thử (Test split)** | `TEST_OPENED = false`, `TEST_READ_COUNT = 0` | `test_opened = false`, 0 lượt truy cập | **ĐẠT YÊU CẦU (CHƯA TRUY CẬP TẬP TEST)** |
| **Độ nguyên vẹn mô hình nền tảng** | `NEW_BACKBONE_OPTIMIZER_STEPS = 0` | 0 bước huấn luyện backbone mới | **ĐẠT YÊU CẦU** |
| **Kế toán tối ưu hóa đầu dò (Probe)** | Minh bạch chính xác số bước probe | 82.200 bước tối ưu probe độc lập | **KHỚP SỐ LIỆU BÁO CÁO** |
| **Quy chuẩn ký tự gạch ngang dài** | Tuyệt đối không chứa ký tự em-dash `—` | 0 ký tự `—` trên toàn văn bản | **ĐẠT YÊU CẦU** |
| **Phát ngôn suy diễn thống kê** | Không tuyên bố ý nghĩa thống kê cho N=3 | Báo cáo trung bình ± độ lệch chuẩn mẫu mô tả định lượng | **ĐẠT YÊU CẦU** |
| **Tuyên bố quan hệ nhân quả** | Không khẳng định nhân quả về VICReg/Fusion | Đặt dưới 3 giả thuyết cơ chế mở đường | **ĐẠT YÊU CẦU** |
...
| **Chính sách công thức toán học (OMML)** | 100% công thức dùng Office Math bản địa | **606 nút OMML** (541 đoạn, 65 bảng), 0 mã LaTeX thô | **ĐẠT YÊU CẦU** |

---

## 3. CHÍNH SÁCH CÔNG THỨC TOÁN HỌC BẢN ĐIẠ (MATHEMATICAL EQUATION POLICY AUDIT)
...
1. **Công thức chỉ số dưới / chỉ số trên thực sự:**
   - Các hàm mất mát đồ thị Stage A2: L_{graph}, L_{rel}, L_{node}, L_{time} được bảo toàn nguyên vẹn dưới dạng `<m:sSub>`, không bị làm phẳng thành văn bản thô.
...
3. **Thước đo thống kê và kiểm định biên:**
   ...
   - Giá trị trung bình ± độ lệch chuẩn mẫu: ΔAP = -0.3392 ± 0.0885, 0.6694 ± 0.0468, 0.8142 ± 0.0675 (độ lệch chuẩn mẫu, phân biệt rõ với phương sai biểu diễn ẩn Var(z)).
...
4. **Kiểm toán mã cấm và cấu trúc:**
   - Rà soát có chủ đích không phát hiện mã nguồn LaTeX thô như `\frac`, `\Delta`, `\in`, `\mathbb`, `\text{` trong văn bản chạy.
   ...
   - Ghi nhận 606 nút OMML hợp lệ trong cấu trúc tài liệu DOCX.

---

## 4. CẤU TRÚC VĂN BẢN VÀ CÁC NỘI DUNG TÍCH HỢP

Công trình giữ nguyên kiến trúc **3 Chương chuẩn** của chuyên đề chuyên sâu, không tự ý chia nhỏ thành 4 hoặc 5 chương:
...
- Xác nhận các biến kiểm soát chiến dịch ghi nhận tập Kiểm thử (Test split) chưa từng được truy cập (test_opened = false, test_reads = 0).

### 4.2. Mục 3.2.3: Đánh giá chuẩn hóa V3 hạ nguồn cho các mô hình xác nhận Sequence và Multi-View
- **Bảng 3.6:** Kết quả đánh giá hạ nguồn V3 trên 100% tập Validation cho bộ ba xác nhận triển vọng:
  - Sequence-Only (Seeds 42, 7, 999): AP = 1.0000, ROC-AUC = 1.0000, Var(z) = 0.005462 ± 0.000821 (độ lệch chuẩn mẫu).
  - Multi-View Aligned VICReg (Seeds 42, 7, 999): AP = 0.6608 ± 0.0885, ROC-AUC = 0.8573 ± 0.1204, Var(z) = 0.006069 ± 0.001734 (độ lệch chuẩn mẫu).
- **Bảng 3.7:** Đối sánh hiệu năng cặp đôi và kiểm định biên không thua kém (δ ≥ -0.02):
  ...
  - Trung bình cặp: ΔAP = -0.3392 ± 0.0885 (độ lệch chuẩn mẫu; không thỏa mãn biên không thua kém trên 100% các cặp).
...
- **Giả thuyết H1 (Parameter Semantic Fidelity):**
  ...
  - Phân định ranh giới suy diễn: `H1_MASKING_ABLATION_STATUS = AUXILIARY_OBSERVATION_ONLY`. Xác lập độ nhạy của biểu diễn đóng băng đối với đầu vào tham số dưới can thiệp che cụ thể; không tuyên bố hiểu biết ngữ nghĩa sâu xa hay bảo đảm tính tối ưu thông tin.
- **Giả thuyết H2 (Multi-View Alignment & Negative Transfer Prevention):**
  - Trạng thái: `NOT_SUPPORTED_WITHIN_SEQUENCE_COMPARATOR_SCOPE`.
  - Dưới giao thức V3, mô hình đơn chuỗi đạt AP = 1.0000, trong khi mô hình đa góc nhìn đạt AP = 0.6608 (suy giảm trung bình -0.3392 AP).
  - **Bảng 3.9:** Kiểm tra độ nhạy bỏ tham số Sequence: Khi che toàn bộ tham số, Sequence vẫn duy trì AP 1.0000 trên cả 3 hạt giống, xác nhận `H2_NEGATIVE_RESULT_ROBUST_TO_THIS_PARAMETER_INPUT_REMOVAL_CHECK`. Kết quả H2 không thay đổi trong kiểm tra độ nhạy cụ thể này; không khái quát hóa thành tính vững chắc phổ quát trước trôi dạt phân phối hay thay đổi kiến trúc.
...
- **Kiểm toán tính toàn vẹn khôi phục Seed 999:** Phân loại là `CONFIRMATORY_WITH_DOCUMENTED_PROTOCOL_DEVIATION`. Toàn bộ trọng số mô hình model_state_dict, optimizer, scheduler, global_step được khôi phục 100%; trạng thái từng phần chỉ đề cập đến việc thiếu trạng thái RNG tuần tự hóa và được tái tạo xác định qua generator.

### 4.4. Mục 3.3.3: Phân tích cơ chế và các giả thuyết giải thích kết quả âm tính H2
- Đặt ra 3 giả thuyết cơ chế mở đường cho nghiên cứu tương lai:
  1. Đặc tính phân tách mạnh của chuỗi mẫu sự kiện HDFS (Sequence đạt AP = 1.0000 dưới giao thức V3).
...
- Cập nhật đầy đủ 8 giới hạn phương pháp luận và thực nghiệm: (1) Quy mô hạt giống xác nhận (N = 3); (2) Giới hạn nhánh đơn đồ thị; (3) Trạng thái tập Kiểm thử (test_opened = false, test_reads = 0); (4) Bất tương thích độ mịn mục tiêu H1 ([L_i, 4] vs z ∈ ℝ^{128}); (5) Nhập nhằng thao tác hóa ngưỡng chống sụp đổ (Var(z) = 0.01000); (6) Sai lệch giao thức khôi phục Seed 999; (7) Phạm vi thực nghiệm trên tập HDFS; (8) Các cơ chế chưa kiểm chứng thực nghiệm.

### 4.6. Phần Kết luận (UH1)
- Nhận diện văn bản đúng chuẩn "Báo cáo chuyên đề chuyên sâu". Phản ánh trung thực kết quả thực nghiệm đạt được, phân định rõ ràng giữa quan sát trực tiếp và suy diễn, ghi nhận đầy đủ kết quả âm tính H2 và các sai lệch thực nghiệm, nêu 3 định hướng phát triển thực tế.

---

## 5. THÔNG SỐ VẬT LÝ VÀ CHUẨN THỂ THỨC MASTER DOCX / PDF

| Thuộc tính | Đặc tính quy định / Quan sát | Thực tế kiểm định trong DOCX/PDF | Đánh giá |
| :--- | :--- | :--- | :--- |
| **Số trang văn bản** | Ghi nhận quan sát thực tế | **114 trang** (Word Statistics: 114) | **PHÙ HỢP** |
| **Hướng trang (Orientation)** | 100% Portrait (Đứng) | **100% Portrait** (114/114 trang) | **ĐẠT YÊU CẦU** |
| **Kích thước khổ giấy** | A4 (210 × 297 mm) | **595.3 × 841.9 pt** (A4 chuẩn xác) | **ĐẠT YÊU CẦU** |
| **Font chữ & Cỡ chữ** | Times New Roman, 14 pt | Times New Roman, 14 pt | **ĐẠT YÊU CẦU** |
| **Dãn dòng (Line Spacing)** | 1.5 lines | 1.5 lines (ONE_POINT_FIVE) | **ĐẠT YÊU CẦU** |
| **Thụt lề đầu dòng** | 1.27 cm (0.5 inch) | 1.27 cm (First line indent) | **ĐẠT YÊU CẦU** |
| **Căn lề trang (Margins)** | Trái 3cm; Trên/Dưới/Phải 2cm | Trái 3cm; Trên/Dưới/Phải 2cm | **ĐẠT YÊU CẦU** |
| **Căn lề đoạn văn (Alignment)**| Justified (Đều hai bên) | Justified (WD_ALIGN_PARAGRAPH.JUSTIFY)| **ĐẠT YÊU CẦU** |
| **Bảng biểu Chương 3** | Đánh số thứ tự liên tục 3.1 - 3.9 | Đánh số tự động qua trường SEQ: Bảng 3.1 - 3.9 | **ĐẠT YÊU CẦU** |
| **Mục lục & Danh mục bảng** | Đồng bộ tự động theo trường Word | Cập nhật hoàn tất qua Word COM | **ĐẠT YÊU CẦU** |
| **Dung lượng file PDF** | Ghi nhận quan sát thực tế | **2.419.057 bytes** (~2.4 MB) | **ĐỊNH DẠNG VECTOR XUẤT BẢN** |
...
Quá trình tích hợp bằng chứng chiến dịch Nineplus V3 vào bản thảo Chuyên đề chuyên sâu (Master Document) đã hoàn tất với tinh thần liêm chính học thuật và độ chính xác kỹ thuật cao:
...
- Biểu thức toán học được biểu diễn bằng Office Math (OMML) bản địa (606 nút OMML ghi nhận), rà soát có chủ đích không phát hiện mã LaTeX thô trong văn bản chạy.
...
- Tập Kiểm thử (Test split) tiếp tục được bảo toàn trạng thái chưa truy cập (test_opened = false, test_reads = 0).
- Toàn bộ tài liệu Master DOCX và PDF đã được biên dịch, làm mới trường tự động và định dạng theo quy chuẩn thể thức báo cáo chuyên đề.
